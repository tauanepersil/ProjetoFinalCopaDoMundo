    ...
    <ImageBackground source={Estadio} style={styles.background} resizeMode="cover">
      <Fragment>
        ...
        <View>
            ...
            {stateGrupoA.map((e) => (
              <Text key={e.idPais} style={styles.groupA}>{e.nome}</Text>
            ))}
            <Text style={[styles.title, styles.grid]}>GRUPO B</Text>
            {stateGrupoB.map((e) => (
              <Text key={e.idPais} style={styles.groupB}>{e.nome}</Text>
            ))}
            <Text style={[styles.title, styles.grid]}>GRUPO C</Text>
            {stateGrupoC.map((e) => (
              <Text key={e.idPais} style={styles.groupC}>{e.nome}</Text>
            ))}
            <Text style={[styles.title, styles.grid]}>GRUPO D</Text>
            {stateGrupoD.map((e) => (
              <Text key={e.idPais} style={styles.groupD}>{e.nome}</Text>
            ))}
            <Text style={[styles.title, styles.grid]}>GRUPO E</Text>
            {stateGrupoE.map((e) => (
              <Text key={e.idPais} style={styles.groupE}>{e.nome}</Text>
            ))}
            <Text style={[styles.title, styles.grid]}>GRUPO F</Text>
            {stateGrupoF.map((e) => (
              <Text key={e.idPais} style={styles.groupF}>{e.nome}</Text>
            ))}
            <Text style={[styles.title, styles.grid]}>GRUPO G</Text>
            {stateGrupoG.map((e) => (
              <Text key={e.idPais} style={styles.groupG}>{e.nome}</Text>
            ))}
            <Text style={[styles.title, styles.grid]}>GRUPO H</Text>
            {stateGrupoH.map((e) => (
              <Text key={e.idPais} style={styles.groupH}>{e.nome}</Text>
            ))}
            <View style={styles.button}>
